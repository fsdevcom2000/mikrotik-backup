...
- [ ] Email-уведомления
- [ ] Webhooks (POST-запросы)

---

### ⚠️ Область применения и ограничения

Данный проект разработан исключительно для создания резервных копий. Функционал восстановления выходит за рамки проекта, отсутствует и никогда не будет добавлен (WONTFIX). Все процедуры по восстановлению выполняются администратором вручную с помощью штатных инструментов RouterOS, таких как WinBox, SSH или Netinstall.
Автоматическое восстановление - крайне рискованная операция, которая легко может привести к падению всей сети или полной потере доступа к оборудованию.
